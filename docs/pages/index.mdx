...
export function AnimatedCode({ code }) {
  ...
  const fastForwardIndex = code.indexOf('txHash');
  ...
  useEffect(() => {
    ...
    const animateCode = async () => {
      ...
      // Progress bar and submission animation
      const startTime = Date.now();
      const duration = 2500; // 2.5 seconds progress bar addition syncing with SubmissionTx
      ...
      await new Promise(resolve => setTimeout(resolve, 4750)); // Remaining time to total 7 seconds
      if (isMounted) {
        setDisplayedCode('');
          ...
          <div style={{ color: '#b3b3b3', marginBottom: '8px', display: 'flex', alignItems: 'center' }}>
            ...
            <span
              style={{
                ...
                textShadow: isGlowing ? '0 0 5px rgba(86, 156, 214, 0.5)' : 'none',
              }}
            >
              run
            </span>
            <span
              style={{
                marginLeft: '8px',
                transition: 'text-shadow 0.3s ease-in-out',
                textShadow: isGlowing ? '0 0 5px rgba(255, 255, 255, 0.5)' : 'none',
              }}
            >
              make_payment.ts
            </span>
