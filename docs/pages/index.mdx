...
export function AnimatedCode({ code }) {
  ...
  const getRandomTypingDelay = () => Math.random() * 50 + 20; // 20-70ms delay

  const lineCount = code.split('\n').length;
  const lineHeight = 1.35; // em
  const maxHeight = `${lineCount * lineHeight}em`;
  ...
  useEffect(() => {
    ...
    const animateCode = async () => {
      ...
      for (let i = 0; i < code.length; i++) {
        ...
        if (code[i] === '\n') {
          await new Promise(resolve => setTimeout(resolve, 500)); // Pause for 500ms at line breaks
        } else {
  ...
  const highlightCode = (code) => {
    ...
    return code.split('\n').map((line, lineIndex) => (
      <div key={lineIndex}>
        ...
        {lineIndex === code.split('\n').length - 1 && <Cursor show={showCursor} />}
      </div>
    ));
  };
  ...
  const Cursor = ({ show }) => (
    <span style={{
      borderLeft: show ? '2px solid #fff' : 'none',
      marginLeft: '2px',
  ...
  const currentHeight = `${(displayedLines + (displayedCode.endsWith('\n') ? 0 : 1)) * lineHeight}em`;
